...
class WebSocketServer
{
	...
	function __construct( $config, $bufferLength = 2048 )
	{
		$addr = $config['address'];
		$port = $config['port'];
	...
	{
		$k = $this->search( $user->socket );
		$eventreturn = array( 'k' => $k, 'msg' => $message, 'user' => $user );
	...
	public function send( $user, $message, $messageType = 'text' )
	{
		if ( $user->handshake )
		{
			$message = $this->frame( $message, $user, $messageType );
